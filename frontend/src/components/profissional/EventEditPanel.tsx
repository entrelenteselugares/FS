      ...
      <div className="relative w-full h-full sm:h-[90vh] max-w-xl flex flex-col border-none sm:border border-theme-border sm:rounded-[32px] overflow-hidden shadow-2xl z-[10000] bg-theme-card">
        ...
        <div className="px-7 pt-7 pb-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-[0.25em] opacity-35">Painel de Entrega</p>
              <h2 className="text-lg font-bold uppercase leading-none mt-0.5" style={{ color: T.text }}>
              ...
              <button
                ...
                className={`flex-1 py-2.5 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${
                  activeTab === tab
                    ...
                    : "text-theme-muted hover:text-brand-text bg-theme-bg-muted"
                }`}
              >
          ...
          {activeTab === "SETUP" && (
            <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-200">
                ...
                <>
                  ...
                  <div className="p-4 bg-theme-bg-muted rounded-2xl border border-theme-border flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[9px] font-bold uppercase tracking-widest opacity-50 mb-1">Data do Evento</p>
                      <input
                        type="date"
                        value={date}
                        onChange={(e) => setDate(e.target.value)}
                        className="w-full bg-transparent border-none outline-none text-sm font-bold text-theme-text"
                      />
                    </div>
                    {date && <span className="text-[9px] font-bold text-brand-tactical uppercase">✓</span>}
                  </div>
                  ...
                  <div className="p-4 bg-theme-bg-muted rounded-2xl border border-theme-border space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-brand-info flex items-center justify-center shrink-0">
                          <Layers size={18} className="text-brand-info" />
                        </div>
                        <div>
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${editingLr ? "bg-brand-tactical text-brand-text" : "bg-theme-bg text-theme-muted hover:text-brand-text border border-theme-border"}`}
                        >
                          {editingLr ? "OK" : "Editar"}
                        </button>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${editingDr ? "bg-brand-tactical text-brand-text" : "bg-theme-bg text-theme-muted hover:text-brand-text border border-theme-border"}`}
                        >
                          {editingDr ? "OK" : "Editar"}
                        </button>
                      </div>
              ...
              <div className="p-4 bg-theme-bg-muted rounded-2xl border border-theme-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${sellPhotos ? "bg-brand-tactical/10" : "bg-brand-danger/10"}`}>
                    <Check size={18} className={sellPhotos ? "text-theme-brand" : "text-brand-danger opacity-50"} />
                  </div>
                  <div>
                ...
                <button
                  ...
                  className={`w-12 h-6 rounded-full relative transition-colors ${sellPhotos ? "bg-brand-tactical/50" : "bg-theme-border"}`}
                >
              ...
              {sellPhotos && (
                <div className="p-4 bg-theme-bg-muted/50 rounded-2xl border border-theme-border/50 space-y-4 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="space-y-4">
                    <div>
                      <h4 className="text-[10px] font-bold uppercase tracking-widest text-brand-tactical mb-2">💰 Votação do Preço da Foto</h4>
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1 flex flex-col gap-1.5">
                        <label className="text-[9px] font-bold uppercase tracking-widest opacity-50 font-bold">Sua Sugestão de Preço (R$)</label>
                        <input
                          type="number"
                          value={mySuggestion}
                      ...
                      <button
                        ...
                        className="px-5 self-end py-3 bg-brand-tactical text-brand-text text-[10px] font-bold uppercase tracking-widest rounded-xl hover:brightness-110 active:scale-[0.98] transition-all h-[44px]"
                      >
                        Sugerir
                      </button>
                    </div>
                    ...
                    {votingStatus && votingStatus.projections && (
                      <div className="space-y-2">
                        <label className="text-[9px] font-bold uppercase tracking-widest opacity-50 block font-bold">📈 Projeção de Faturamento ({votingStatus.guests} Convidados)</label>
                          ...
                          <table className="w-full text-left border-collapse text-[9px]">
                            <thead>
                              <tr className="bg-theme-bg-muted border-b border-theme-border text-theme-muted uppercase tracking-wider">
                                <th className="p-2.5 font-bold">Conversão</th>
                                <th className="p-2.5 font-bold">Compradores</th>
                                <th className="p-2.5 font-bold">Contratante</th>
                                <th className="p-2.5 font-bold">Equipe</th>
                                <th className="p-2.5 font-bold">Recomendado</th>
                              </tr>
                            </thead>
                    ...
                    {votingStatus && (
                      <div className="space-y-2">
                        <label className="text-[9px] font-bold uppercase tracking-widest opacity-50 block font-bold">🗳 Escolha a melhor opção</label>
                          ...
                          <button
                            ...
                            className={`py-2 px-3 text-[9px] font-black uppercase rounded-lg border transition-all ${
                              votingStatus.votes[user?.id || ""] === "OWNER"
                                ? "bg-brand-tactical border-brand-tactical text-brand-text animate-pulse"
                                : "bg-theme-bg border-theme-border text-brand-text hover:border-theme-muted disabled:opacity-30"
                            }`}
                          >
                            Contratante
                          </button>
                    ...
                    {votingStatus && Object.keys(votingStatus.votes).length > 0 && (
                      <div className="p-3 bg-theme-bg-muted rounded-xl text-[9px] space-y-1">
                        ...
                        {votingStatus.winner && (
                          <div className="pt-2 border-t border-theme-border/30 mt-2 font-bold text-brand-tactical flex justify-between uppercase tracking-wider text-[10px]">
              ...
              <div className="p-4 bg-brand-tactical/8 rounded-2xl border border-brand-tactical/20 space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 bg-brand-tactical text-brand-text text-[9px] font-bold uppercase rounded-xl hover:brightness-110 transition-all shrink-0"
                  >
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              <div className="p-4 bg-theme-bg-muted rounded-2xl border border-theme-border flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-theme-text">Você</p>
                </div>
                <span className="ml-auto text-[9px] font-bold px-2 py-1 rounded-full bg-brand-tactical/15 text-brand-tactical uppercase">
              ...
              {/* Editor */}
              <div className="space-y-3">
                <p className="text-[9px] font-bold uppercase tracking-widest opacity-40 pl-1">Editor de Vídeo / Pós-Produção</p>
                  ...
                  <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                    <div className="p-4 bg-brand-tactical/8 border border-brand-tactical/25 rounded-2xl flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="p-2 text-theme-muted hover:text-brand-danger hover:bg-brand-danger/10 transition-all rounded-xl"
                    ...
                    {/* Editor Services */}
                    <div className="pt-2">
                      <p className="text-[9px] font-bold uppercase tracking-widest opacity-40 pl-1 mb-2">Serviços Oferecidos</p>
                      {loadingServices ? (
                        <p className="text-[10px] text-theme-muted pl-1">Carregando serviços...</p>
                      ) : editorServices.length === 0 ? (
                        <p className="text-[10px] text-theme-muted pl-1">Este editor não cadastrou serviços com preço fixo.</p>
                      ) : (
                        <div className="space-y-2">
                          {editorServices.map((svc) => (
                            <div key={svc.id} className="p-3 bg-theme-bg-muted border border-theme-border rounded-xl flex items-center justify-between">
                              <div>
                                <p className="text-[11px] font-bold text-theme-text uppercase">{svc.name}</p>
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 bg-brand-tactical text-brand-text text-[9px] font-bold uppercase tracking-wider rounded-lg hover:brightness-110 transition-all"
                              >
                                Contratar
                              </button>
                            </div>
                    ...
                    {searchResults.length > 0 && (
                      <div className="bg-theme-bg-muted border border-theme-border rounded-2xl overflow-hidden divide-y divide-theme-border/40">
                        {searchResults.map((pro) => (
                          ...
                          >
                            <div className="w-8 h-8 rounded-full bg-theme-bg border border-theme-border flex items-center justify-center text-[11px] font-bold text-theme-muted shrink-0">
                              {(pro.nome || "?")[0].toUpperCase()}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-bold text-theme-text truncate">{pro.nome}</p>
                              <p className="text-[10px] text-theme-muted truncate">{pro.email}</p>
                            </div>
                            <span className="text-[9px] font-bold text-brand-tactical uppercase shrink-0">Atribuir →</span>
                          </button>
                        ))}
                      </div>
                    )}
              ...
              <div className="space-y-4 pt-4 border-t border-theme-border/40">
                <p className="text-[9px] font-bold uppercase tracking-widest opacity-40 pl-1">Outros Membros da Equipe</p>
                  ...
                  <div className="space-y-2">
                    {teamMembers.map((member) => (
                      <div key={member.id} className="p-4 bg-theme-bg-muted border border-theme-border rounded-2xl flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-theme-text uppercase">{member.user?.nome}</p>
                ...
                <div className="space-y-3 bg-theme-bg-muted/30 p-4 border border-theme-border/50 rounded-2xl">
                  <p className="text-[9px] font-bold uppercase tracking-widest opacity-40">Adicionar Membro à Equipe</p>
                  ...
                  {memberSearchResults.length > 0 && (
                    <div className="bg-theme-bg border border-theme-border rounded-xl overflow-hidden divide-y divide-theme-border/40">
                      {memberSearchResults.map((pro) => (
                        ...
                        >
                          <div className="w-7 h-7 rounded-full bg-theme-bg-muted border border-theme-border flex items-center justify-center text-[10px] font-bold text-theme-muted shrink-0">
                            {(pro.nome || "?")[0].toUpperCase()}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-theme-text truncate">{pro.nome}</p>
                          ...
                          <span className="text-[9px] font-bold text-brand-tactical uppercase shrink-0">Adicionar +</span>
                        </button>
                      ))}
                    </div>
                  )}
          ...
          {activeTab === "PRINT" && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-200">
              <div className="p-6 bg-brand-tactical/10 border border-brand-tactical/20 rounded-3xl flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-widest text-brand-tactical mb-2">Kit de Impressão Phygital</h3>
                ...
                <button
                  ...
                  className="w-full py-4 mt-2 bg-brand-tactical text-brand-text text-[10px] font-bold uppercase tracking-widest rounded-xl hover:brightness-110 transition-all flex items-center justify-center gap-2"
                >
        ...
        <div className="px-7 py-5 border-t border-theme-border flex items-center gap-3 shrink-0 bg-theme-bg-muted/50">
          <button onClick={onClose} className="px-6 py-3.5 text-theme-muted text-[10px] font-bold uppercase tracking-widest hover:text-theme-text transition-all rounded-xl">
            Cancelar
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 py-3.5 bg-brand-tactical text-brand-text text-[11px] font-bold uppercase tracking-[0.2em] hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-3 shadow-lg shadow-brand-tactical/20 rounded-xl disabled:opacity-60"
          >
      ...
      {paymentModalService && (
        <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-theme-card border border-theme-border rounded-[24px] p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <h3 className="text-sm font-bold uppercase text-theme-text mb-1">
              Contratar Serviço
            </h3>
            ...
            <div className="space-y-3 mb-6">
              <label className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-all ${paymentMethod === "CREDITS" ? "border-brand-tactical bg-brand-tactical/5" : "border-theme-border hover:border-theme-muted"}`}>
                ...
                <div>
                  <p className="text-[11px] font-bold uppercase text-theme-text">Usar Créditos</p>
              ...
              <label className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-all ${paymentMethod === "MP" ? "border-brand-tactical bg-brand-tactical/5" : "border-theme-border hover:border-theme-muted"}`}>
                ...
                <div>
                  <p className="text-[11px] font-bold uppercase text-theme-text">Mercado Pago</p>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-3 text-[10px] font-bold uppercase tracking-widest text-theme-muted hover:text-zinc-100 bg-theme-bg-muted hover:bg-black/20 rounded-xl transition-all"
                disabled={hiring}
              >
                Cancelar
              </button>
              <button
                ...
                disabled={hiring}
                className="flex-1 py-3 bg-brand-tactical text-brand-text text-[10px] font-bold uppercase tracking-widest rounded-xl hover:brightness-110 transition-all disabled:opacity-50"
              >
