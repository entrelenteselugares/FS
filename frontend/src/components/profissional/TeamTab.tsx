...
export function TeamTab() {
  ...
  if (loading) {
    return <div className="p-10 text-center text-theme-muted uppercase font-bold text-[10px] tracking-widest">Carregando Rede Técnica...</div>;
  }
      ...
      <div className="lux-card p-10 border-l-4 border-l-brand-tactical rounded-2xl bg-gradient-to-br from-brand-tactical/[0.03] to-transparent relative overflow-hidden shadow-sm">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <p className="text-[11px] font-bold text-theme-muted uppercase tracking-[0.2em] leading-relaxed max-w-3xl">
            Otimize sua operação designando profissionais <span className="text-brand-tactical font-bold underline decoration-brand-tactical/30 underline-offset-4">FIXOS</span> para prioridade máxima em seus eventos ou integrando o pool <span className="text-blue-400 font-bold">ROTATIVO</span> para demandas dinâmicas de rede.
          </p>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
              <div className="px-4 py-3 bg-theme-bg border border-theme-border rounded-xl flex items-center gap-3">
                ...
                <span className="text-[9px] font-bold text-theme-text uppercase tracking-widest">{filteredTeam.length} AGENTES IDENTIFICADOS</span>
              </div>
                  ...
                  <div className="flex-1 p-6 flex flex-col justify-between gap-6">
                    <div className="flex flex-wrap gap-2">
                      {p.services.length > 0 ? p.services.map(s => (
                        <span key={s} className="px-3 py-1 bg-theme-bg border border-theme-border rounded-lg text-[9px] font-bold text-theme-text uppercase tracking-widest group-hover:border-brand-tactical/30 transition-colors shadow-sm truncate max-w-full">
                          {s}
                        </span>
                      )) : (
      ...
      {Object.keys(teamChanges).length > 0 && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] animate-in slide-in-from-bottom-10 duration-500 w-[90%] md:w-auto">
          <div className="bg-theme-bg-muted/90 backdrop-blur-xl border border-brand-tactical/50 rounded-2xl p-6 md:p-8 shadow-2xl flex flex-col md:flex-row items-center gap-6 md:gap-10">
            <p className="text-[10px] font-bold text-theme-text uppercase tracking-[0.3em] text-center md:text-left">
              {Object.keys(teamChanges).length} ALTERAÇÃO(ÕES) PENDENTE(S) NO QUADRO TÁTICO
            </p>
            <button
              disabled={savingTeam}
              onClick={saveTeam}
              className="bg-brand-tactical text-brand-text rounded-xl px-10 py-4 text-[10px] font-bold uppercase tracking-[0.4em] hover:brightness-110 transition-all shadow-lg w-full md:w-auto"
            >
