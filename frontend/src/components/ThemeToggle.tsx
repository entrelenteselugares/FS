import { useTheme } from "../contexts/ThemeContextCore";
import { Moon, Sun } from "lucide-react";



export function ThemeToggle() {
  const { theme, toggle } = useTheme();

  return (
    <button
      onClick={toggle}
      className="p-3 transition-all group relative overflow-hidden"
      style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 0 }}
      title={theme === 'dark' ? "Ativar Modo Diurno" : "Ativar Modo Noturno"}
    >
      <div className="relative z-10 flex items-center justify-center">
        {theme === 'dark' ? (
          <Sun size={14} className="text-brand-tactical" strokeWidth={1.5} />
        ) : (
          <Moon size={14} className="text-theme-text" strokeWidth={1.5} />
        )}
      </div>
      <div className="absolute inset-0 bg-brand-tactical/10 opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity" />
    </button>
  );
}
