    ...
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center px-6 text-center bg-zinc-950">
        ...
        <div className="bg-zinc-900 border border-white/10 px-6 py-4 rounded-xl mb-6">
          <span className="text-[9px] font-bold text-theme-brand uppercase tracking-widest block mb-1">Protocolo</span>
          <span className="text-xl font-bold text-theme-text tracking-widest">ORC-{props.createdQuoteId.slice(-6).toUpperCase()}</span>
        </div>
        ...
        <div className="flex flex-col gap-3 w-full max-w-xs mt-8">
          ...
          <button
            ...
            className="w-full py-3.5 border border-white/10 text-theme-muted hover:text-theme-text text-[11px] font-bold uppercase tracking-widest rounded-full"
          >
            Voltar ao Início
          </button>
        </div>
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col font-b selection:bg-brand-tactical selection:text-brand-text pb-24" style={{ backgroundColor: THEME.bg, color: THEME.text }}>
      ...
      <div className="sticky top-0 z-50 bg-zinc-950/80 backdrop-blur-md px-4 py-3 flex items-center border-b border-white/5">
        {step > 1 && (
          <button onClick={prevStep} className="p-1 -ml-1 mr-2 text-theme-muted hover:text-theme-text active:scale-95 transition-transform">
            <ChevronLeft size={20} />
          </button>
        )}
        <div className="flex-1">
          <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-theme-brand">Passo {step} de 4</div>
          <div className="text-sm font-medium text-theme-text/90">
      ...
      {flowType && (
        <div className="bg-brand-tactical/10 border-b border-brand-tactical/20 px-4 py-3 flex items-center justify-between shadow-xl">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-[9px] font-bold uppercase tracking-widest text-theme-brand">Jornada Selecionada</div>
          ...
          <button onClick={() => setStep(0)} className="text-[9px] font-bold uppercase tracking-widest text-theme-muted hover:text-theme-text border-b border-white/10 pb-0.5">
            Trocar
          </button>
        </div>
      )}
          ...
          >
              ...
              <div className="space-y-6">
                ...
                {locationType === "PARTNER" && (
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider">Escolha o Estabelecimento</label>
                    <div className="flex flex-col gap-2">
                      {partners.map((p: any) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className={`text-sm font-medium ${selectedPartnerId === p.id ? "text-theme-brand" : "text-theme-subtle"}`}>{p.name}</div>
                            <div className="text-[10px] text-theme-muted uppercase tracking-widest">{p.city}</div>
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                {locationType === "OTHER" && (
                  <div className="space-y-1.5 pt-2">
                    <label className="text-xs font-semibold text-theme-brand uppercase tracking-wider ml-1 flex items-center gap-1"><MapPin size={12}/> CEP do Local</label>
                    <input
                      type="text"
                ...
                <div className="space-y-1.5 pt-2">
                  <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider ml-1">Data do Evento</label>
                  <input
                    type="date"
                    value={eventDate}
            ...
            {step === 2 && (
              <>
                {(locationType === "PARTNER" && currentPartner?.eventTypes && currentPartner.eventTypes.length > 0) ? (
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider">Tipo de Evento</label>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider">Servi�os Oferecidos</label>
                  <div className="flex flex-col gap-2">
                    {availableServices.length === 0 ? (
                      <div className="p-4 text-center text-xs text-theme-muted bg-zinc-900/30 rounded-lg border border-white/5">Nenhum servi�o dispon�vel para este tipo de evento.</div>
                    ) : (
                          ...
                          >
                            ...
                            <div className="flex-1 flex flex-col justify-center">
                              <div className="flex items-center justify-between">
                                <span className={`text-sm font-medium ${isSelected ? "text-theme-brand" : "text-theme-subtle"}`}>{svc.name}</span>
                                {getServicePrice && (
                              ...
                              {svc.description && (
                                <span className="text-[10px] text-theme-muted mt-0.5 line-clamp-1">{svc.description}</span>
                              )}
                            </div>
                          </button>
                        );
            ...
            {step === 3 && (
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider ml-1">Convidados (Aprox.)</label>
            ...
            {step === 4 && (
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider ml-1">Nome Completo</label>
                  <input
                    type="text"
                    value={name}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider ml-1">E-mail</label>
                  <input
                    type="email"
                    value={email}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider ml-1">WhatsApp</label>
                  <input
                    type="tel"
                    value={whatsapp}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider ml-1">Link de Venda de Ingressos (Opcional)</label>
                  <input
                    type="url"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-theme-muted uppercase tracking-wider ml-1">Seu Cupom Promocional (Opcional)</label>
                  <input
                    type="text"
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-zinc-950/90 backdrop-blur-md border-t border-white/5 pb-8">
        <div className="flex items-center gap-3">
          {totalPrice > 0 && (
            <div className="flex-1">
              <div className="text-[10px] text-theme-muted uppercase font-bold tracking-widest">Total Estimado</div>
              <div className="text-lg font-bold text-theme-brand">R$ {totalPrice.toFixed(2)}</div>
            </div>
          )}
