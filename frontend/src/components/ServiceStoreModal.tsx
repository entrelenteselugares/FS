            ...
            <div className="space-y-6">
              <div className="bg-zinc-800/50 p-4 border border-white/10 rounded-xl mb-4">
                 ...
                 <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
                    <span className="text-xs uppercase font-bold tracking-widest text-zinc-500">Total</span>
                    <span className="text-xl font-bold text-emerald-500">R$ {selectedService.basePrice.toFixed(2).replace('.', ',')}</span>
                 </div>
              </div>

              <div>
                <label className="block text-[10px] text-zinc-400 uppercase tracking-widest font-bold mb-2">Link de Referência (Opcional)</label>
              ...
              <div>
                <label className="block text-[10px] text-zinc-400 uppercase tracking-widest font-bold mb-2">Sugestão de Trilha Sonora (Opcional)</label>
              ...
              <div>
                <label className="block text-[10px] text-zinc-400 uppercase tracking-widest font-bold mb-2">Observações e Orientações (Opcional)</label>
              ...
              <div className="pt-4 border-t border-white/10 flex justify-end gap-3">
                 ...
                 <button
                    ...
                    className="bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-black px-6 py-2 rounded-lg font-bold uppercase tracking-widest text-xs transition-all flex items-center gap-2 active:scale-95"
                 >
              ...
              {services.map((service) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="px-2 py-1 bg-theme-bg-muted text-zinc-300 text-[9px] font-bold uppercase tracking-widest rounded">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/5">
                    <div>
                      <span className="text-[10px] text-zinc-500 uppercase font-bold block">Investimento</span>
                      <span className="text-xl font-bold text-emerald-500">R$ {service.basePrice.toFixed(2).replace('.', ',')}</span>
                    </div>
                    
                    <button
                      onClick={() => setSelectedService(service)}
                      className="bg-emerald-500 hover:bg-emerald-400 text-black px-4 py-2 rounded-lg font-bold uppercase tracking-widest text-[10px] transition-all flex items-center gap-2 active:scale-95"
                    >
                      <Plus size={14} />
                      Contratar
                    </button>
