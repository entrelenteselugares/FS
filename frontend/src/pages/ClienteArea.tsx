...
export default function ClienteArea() {
 ...
 {m.value}
  ...
  <div className="lg:hidden mb-6 space-y-4">
    ...
    {/* ── Grupo: Conta ── */}
    <div>
      <p className="text-[9px] font-black uppercase tracking-[0.3em] text-brand-tactical mb-2 px-1">Minha Conta</p>
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: "Compras", icon: <ShoppingBag size={20} />, tab: "files" },
          { label: "Carteira", icon: <Wallet size={20} />, tab: "wallet" },
          { label: "Indique", icon: <Users size={20} />, tab: "affiliate" },
          { label: "Meus Dados", icon: <User size={20} />, tab: "profile" },
          { label: "Meus Álbuns", icon: <Lock size={20} />, onClick: () => navigate("/meus-albuns") },
        ].map((it) => {
          const active = it.tab ? activeTab === it.tab : false;
          return (
            <button
              key={it.label}
              onClick={it.onClick ?? (() => it.tab && handleTabChange(it.tab))}
              className={`flex flex-col items-center gap-2 p-3 rounded-2xl border transition-all ${
                active
                  ? "bg-brand-tactical/15 border-brand-tactical text-brand-tactical"
                  : "bg-[var(--bg-card)] border-[var(--border)] text-[var(--text-muted)] hover:border-brand-tactical hover:text-brand-tactical"
              }`}
            >
              {it.icon}
              <span className="text-[9px] font-bold uppercase tracking-wider leading-tight text-center">{it.label}</span>
            </button>
          );
        })}
      </div>
    ...
    {/* ── Grupo: Painel Profissional (só aparece se for pro verificado) ── */}
    {(() => {
      const isPro = (user?.role === "PROFISSIONAL" || user?.role === "FRANCHISEE" || !!user?.franchiseProfile)
      ...
      return (
        <div>
          <p className="text-[9px] font-black uppercase tracking-[0.3em] text-brand-tactical mb-2 px-1">Painel Profissional</p>
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: "Agenda", icon: <Play size={20} />, tab: "agenda" },
              { label: "Portfólio", icon: <ImageIcon size={20} />, tab: "portfolio" },
              { label: "Serviços", icon: <Briefcase size={20} />, tab: "servicos" },
              { label: "Ficha Técnica", icon: <Settings size={20} />, tab: "perfil" },
              { label: "Financeiro", icon: <DollarSign size={20} />, tab: "financeiro" },
            ].map((it) => {
              const active = activeTab === it.tab;
              return (
                <button
                  key={it.label}
                  onClick={() => handleTabChange(it.tab)}
                  className={`flex flex-col items-center gap-2 p-3 rounded-2xl border transition-all ${
                    active
                      ? "bg-brand-tactical/15 border-brand-tactical text-brand-tactical"
                      : "bg-[var(--bg-card)] border-[var(--border)] text-[var(--text-muted)] hover:border-brand-tactical hover:text-brand-tactical"
    ...
    {/* ── Grupo: Gestão de Franquia ── */}
    {(user?.role === "FRANCHISEE" || !!user?.franchiseProfile) && (
      <div>
        <p className="text-[9px] font-black uppercase tracking-[0.3em] text-brand-tactical mb-2 px-1">Gestão de Franquia</p>
        <div className="grid grid-cols-3 gap-2">
          {[
            ...(user?.role === "FRANCHISEE" ? [{ label: "Franquia", icon: <LayoutDashboard size={20} />, onClick: () => navigate("/franquia") }] : []),
            { label: "Rede Técnica", icon: <Users size={20} />, tab: "equipe" },
            { label: "Franquia Print", icon: <Printer size={20} />, tab: "franquia" },
          ].map((it: any) => {
            const active = it.tab ? activeTab === it.tab : false;
            return (
              <button
    ...
    {/* ── Grupo: Unidade / Cartório ── */}
    {(user?.role === "CARTORIO" || user?.role === "UNIDADE") && user?.verificationStatus === "APPROVED" && (
      <div>
        <p className="text-[9px] font-black uppercase tracking-[0.3em] text-brand-tactical mb-2 px-1">Gestão da Unidade</p>
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: "Agenda", icon: <Play size={20} />, tab: "agenda" },
            { label: "Financeiro", icon: <DollarSign size={20} />, tab: "financeiro" },
            { label: "Rede Técnica", icon: <Users size={20} />, tab: "equipe" },
            { label: "Configuração", icon: <Settings size={20} />, tab: "configuracoes" },
          ].map((it) => {
            const active = activeTab === it.tab;
            return (
              <button
    ...
    {user?.role === "ADMIN" && (
      <button
        ...
        className="w-full flex items-center gap-3 p-3 rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] text-[var(--text-muted)] hover:border-brand-tactical hover:text-brand-tactical transition-all"
      >
        <ShieldCheck size={18} />
    ...
    <button
      ...
      onClick={logout}
      className="w-full flex items-center justify-center gap-3 p-3 rounded-2xl border border-red-900/40 text-red-400/70 hover:border-red-500 hover:text-red-400 hover:bg-red-500/5 transition-all"
    >
      ...
      <span className="text-[10px] font-bold uppercase tracking-[0.3em]">Encerrar Sessão</span>
    </button>
  </div>
