    ...
    <div className="fixed inset-0 z-[5000] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-xl">
      ...
      >
        ...
        <div className="px-3 md:px-6 pt-6 pb-4 border-b border-white/5">
          <p className="text-[9px] font-bold text-brand-tactical uppercase tracking-widest mb-1">Taxa de Reserva</p>
          <h2 className="text-xl font-heading font-bold text-theme-text uppercase">
            Reservar {prof.nome}
          </h2>
        </div>
        ...
        <div className="px-3 md:px-6 py-5 space-y-4">
          ...
          <div className="flex items-center justify-between bg-white/5 rounded-xl p-4">
            <div>
              <p className="text-[9px] font-bold text-theme-muted uppercase tracking-widest mb-1">Pacote</p>
              <p className="text-sm font-bold text-theme-text">{state.service.name}</p>
            </div>
            <div className="text-right">
              <p className="text-xl font-heading font-bold text-brand-tactical">R$ {fee}</p>
              <p className="text-[10px] text-theme-muted font-bold uppercase">taxa (20%)</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 bg-brand-tactical/5 border border-brand-tactical/15 rounded-xl p-3.5">
            ...
            <p className="text-[9px] text-theme-muted leading-relaxed font-medium">
              Após o pagamento, o WhatsApp de <strong className="text-theme-text">{prof.nome}</strong> será liberado por e-mail. O restante é pago diretamente ao profissional.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 px-3 md:px-6 pb-6">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-3.5 border border-white/10 rounded-xl text-theme-muted text-[10px] font-bold uppercase tracking-widest hover:border-white/20 transition-all"
          >
            Cancelar
          </button>
          <button
...
export default function ProfissionalProfilePage() {
  ...
  if (!prof) {
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center text-theme-muted">
        ...
        <div className="relative h-52 md:h-72 bg-zinc-900 overflow-hidden">
          {prof.coverImageUrl ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 flex items-center justify-center">
              <Camera size={56} className="text-theme-text" />
            </div>
          )}
            ...
            <div className="flex flex-col sm:flex-row gap-5 sm:items-end">
              ...
              <div className="relative flex-shrink-0">
                <div className="w-28 h-28 md:w-32 md:h-32 rounded-2xl bg-zinc-800 overflow-hidden border-4 border-zinc-950 shadow-2xl">
                  {prof.profileImageUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-theme-muted bg-zinc-900 text-3xl font-bold">
                ...
                {prof.isVerified && (
                  <div className="absolute -bottom-1 -right-1 bg-brand-tactical w-7 h-7 rounded-lg flex items-center justify-center border-2 border-zinc-950">
                    <ShieldCheck size={13} className="text-theme-text" />
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="text-3xl md:text-4xl font-heading font-bold text-theme-text uppercase leading-none">
                    {prof.nome}
                  </h1>
                  {prof.isSubscriber && (
                ...
                {city && (
                  <div className="flex items-center gap-3 flex-wrap">
                    <p className="flex items-center gap-1.5 text-xs text-theme-muted font-semibold">
                      <MapPin size={12} className="text-brand-tactical" /> {city}
                    </p>
                    {prof.serviceRadiusKm && (
                      <span className="text-[9px] text-theme-muted font-bold uppercase tracking-widest bg-zinc-900 border border-zinc-800 px-2 py-0.5 rounded-full">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {prof.services.slice(0, 5).map(s => (
                    <span key={s} className="px-2.5 py-1 bg-zinc-800/80 border border-zinc-700/50 text-[10px] font-bold text-theme-subtle uppercase tracking-wider rounded-full">
                      {s}
                    </span>
                  ))}
            ...
            <div className="mt-5 grid grid-cols-4 gap-2 bg-zinc-900/60 border border-zinc-800/60 rounded-2xl p-3">
              {[
                ...
                <div key={s.label} className="text-center py-2">
                  <p className={`text-xl font-heading font-black italic leading-none ${s.accent ? "text-brand-tactical" : "text-theme-text"}`}>
                    {s.value}
                  </p>
                  <p className="text-[9px] text-theme-muted uppercase tracking-widest font-bold mt-1">{s.label}</p>
                </div>
              ))}
            </div>

            {prof.otherHabilities && (
              <p className="mt-4 text-sm text-theme-muted leading-relaxed max-w-2xl">
                {prof.otherHabilities}
              </p>
            )}
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 space-y-14 pb-20">
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-sm font-heading font-bold text-theme-text uppercase tracking-widest">Pacotes Disponíveis</h2>
            <div className="flex-1 h-px bg-zinc-800" />
            <span className="text-[9px] text-theme-muted font-bold uppercase tracking-widest">
          ...
          {prof.proServices.length === 0 ? (
            <div className="py-14 text-center border border-dashed border-zinc-800 rounded-2xl">
              <Camera size={36} className="text-theme-text mx-auto mb-3" />
              <p className="text-theme-muted text-xs uppercase tracking-widest font-bold">Nenhum pacote cadastrado</p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex flex-col md:flex-row items-start md:justify-between gap-1 md:gap-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-[9px] md:text-sm font-heading font-bold text-theme-text uppercase leading-tight line-clamp-3 md:line-clamp-none">{svc.name}</h3>
                        {svc.description && (
                          <p className="hidden md:block text-[10px] text-theme-muted mt-1 leading-relaxed line-clamp-2">{svc.description}</p>
                        )}
                      </div>
                      <div className="flex-shrink-0 md:text-right mt-1 md:mt-0">
                        <p className="text-[10px] md:text-lg font-heading font-bold text-brand-tactical md:text-theme-text leading-none">
                          R$ {activePrice.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                        </p>
                        <p className="hidden md:block text-[9px] text-theme-muted uppercase tracking-widest font-bold mt-0.5">total</p>
                      </div>
                    </div>

                    {/* Meta */}
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-1 md:gap-3 text-[9px] md:text-[9px] text-theme-muted font-bold uppercase mt-1 md:mt-0">
                      <span className="flex items-center gap-1">
                        <Clock size={8} className="md:w-[10px] md:h-[10px]" /> {hrs > 0 ? `${hrs}h` : ""}{mins > 0 ? `${mins}m` : ""}
                      </span>
                      <span className="flex items-center gap-1 text-theme-muted md:text-brand-tactical">
                    ...
                    <div className="mt-auto pt-1 md:pt-2">
                      <div className="hidden md:flex items-center gap-1 text-[10px] text-theme-text font-bold uppercase tracking-widest mb-2">
                      ...
                      {user ? (
                        ...
                        <Link
                          ...
                          className="block w-full flex items-center justify-center py-1.5 md:py-3 bg-zinc-800 text-theme-subtle text-center text-[9px] md:text-[9px] font-bold uppercase tracking-widest rounded-lg md:rounded-xl hover:bg-zinc-700 transition-all leading-tight"
                        >
        ...
        {albums.length > 0 && (
          <section>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-sm font-heading font-bold text-theme-text uppercase tracking-widest">Portfólio</h2>
              <div className="flex-1 h-px bg-zinc-800" />
            </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={b.label} className="p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-2xl text-center space-y-2">
                ...
                <p className="text-[10px] font-bold text-theme-text uppercase tracking-wider">{b.label}</p>
                <p className="text-[9px] text-theme-muted font-medium leading-relaxed">{b.desc}</p>
              </div>
            ))}
          </div>
        </section>
