      ...
      >
        ...
        <div className="space-y-3 pl-1">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-brand-tactical ">{quote.priceBase?`R$ ${quote.priceBase.toLocaleString()}`:"S/ VALOR"}</span>
          </div>
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-3 md:gap-6 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <button onClick={()=>setIsNewQuoteModalOpen(true)}
              className="bg-brand-tactical text-black font-display font-bold uppercase tracking-[0.2em] px-3 md:px-6 py-3 text-[10px] flex items-center gap-2 hover:brightness-110 shadow-xl transition-all rounded-lg whitespace-nowrap">
            ...
            >
              ...
              <div className="p-4 md:p-8 md:p-10 border-b border-theme-border flex items-start justify-between shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold uppercase text-theme-text">{selectedQuote.title}</h3>
                    <p className="text-[9px] font-bold uppercase tracking-[0.2em] opacity-40 mt-1">Protocolo: {selectedQuote.id.toUpperCase()}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 md:p-10">
                          ...
                          {data && (
                            <div className="bg-brand-tactical/10 p-4 border border-brand-tactical/20 rounded-xl space-y-3">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-[9px] font-bold text-brand-tactical uppercase tracking-widest ">Análise de Recursos</span>
                              </div>
                              <div className="grid grid-cols-1 gap-2 text-[10px]">
                                <div className="flex justify-between border-b border-brand-tactical/10 pb-1">
                                  <span className="text-theme-subtle ">Equipe Sugerida</span>
                                  <span className="text-theme-text font-bold ">{(data as BudgetBreakdown).STAFF?.map(s => s.LABEL || s.label).join(", ") || "N/A"}</span>
                                </div>
                                <div className="flex justify-between border-b border-brand-tactical/10 pb-1">
                                  <span className="text-theme-subtle ">Markup Aplicado</span>
                                  <span className="text-theme-text font-bold ">{(data as BudgetBreakdown).MARGIN || ((data as BudgetBreakdown).margin)}%</span>
                                </div>
                              </div>
                            </div>
                          )}
                          <div className="bg-theme-bg-muted p-4 border border-theme-border text-[11px] text-theme-text leading-relaxed font-bold whitespace-pre-wrap rounded-lg">{cleanText||"Nenhuma observação adicional."}</div>
                        </div>
                      );
                    })()}
                  </div>
                ...
                {activeTab==="locacao"&&(
                  <div className="space-y-4 animate-in fade-in duration-300">
                    <div className="flex items-center justify-between border-l-2 border-brand-tactical pl-2">
                      <h4 className="text-[9px] font-bold text-theme-text uppercase tracking-widest">Locação de Equipamentos</h4>
                      <span className="text-[9px] font-bold text-brand-tactical uppercase tracking-widest bg-brand-tactical/10 px-2 py-0.5 rounded">Total: R$ {equipTotal.toLocaleString()}</span>
                    </div>
                ...
                {activeTab==="custos"&&(
                  <div className="space-y-4 animate-in fade-in duration-300">
                    <h4 className="text-[9px] font-bold text-theme-text uppercase tracking-widest border-l-2 border-brand-tactical pl-2">Logística e Despesas</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5"><label className="text-[9px] font-bold text-theme-text-muted uppercase tracking-widest">Deslocamento (R$)</label><input type="number" value={transportCost} onChange={e=>setTransportCost(Number(e.target.value))} className="w-full bg-theme-bg border border-theme-border p-2.5 text-[11px] font-bold text-theme-text outline-none focus:border-brand-tactical rounded-lg"/></div>
              ...
              <div className="p-4 md:p-8 md:p-10 bg-theme-bg-muted border-t border-theme-border flex gap-4 shrink-0 rounded-2xl">
                <button onClick={handleReject} className="py-5 px-3 md:px-6 border border-red-500/20 text-red-500 text-[9px] font-bold uppercase tracking-[0.2em] hover:bg-red-500 hover:text-white transition-all rounded-[20px] ">Recusar</button>
                
                {selectedQuote.quoteStatus === "CONVERTED" && (
                  <button onClick={handleArchive} className="flex-1 py-5 bg-zinc-800 text-white text-[11px] font-bold uppercase tracking-[0.3em] hover:bg-zinc-700 transition-all rounded-[20px] flex items-center justify-center gap-4">
                    Finalizar Entrega & Arquivar
                  </button>
                )}

                {selectedQuote.quoteStatus !== "CONVERTED" && selectedQuote.quoteStatus !== "ARCHIVED" && (
                  <button onClick={handleApprove} disabled={finalPrice<=0||approving} className="flex-1 py-5 bg-brand-tactical text-zinc-950 text-[11px] font-bold uppercase tracking-[0.3em] shadow-2xl shadow-brand-tactical/20 hover:brightness-110 transition-all rounded-[20px] flex items-center justify-center gap-4 disabled:opacity-50">
          ...
          <div className="relative w-full max-w-2xl bg-theme-card border border-theme-border rounded-[40px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col h-[90vh]">
            ...
            <div className="p-4 md:p-8 md:p-10 border-b border-theme-border flex items-center justify-between shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold uppercase text-theme-text">Novo Lead</h2>
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-40">Abertura de Protocolo Direto</p>
                </div>
              </div>
            ...
            {/* Footer */}
            <div className="p-4 md:p-8 md:p-10 bg-theme-bg-muted border-t border-theme-border flex gap-4 shrink-0 rounded-2xl">
              <button type="button" onClick={() => setIsNewQuoteModalOpen(false)} className="flex-1 py-5 border border-theme-border text-[11px] font-bold uppercase tracking-[0.3em] text-theme-muted hover:text-white transition-all rounded-[20px] ">Cancelar</button>
              <button 
                type="submit"
                form="new-quote-form"
