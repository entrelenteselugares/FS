...
- _Observação:_ O botão padrão de pagar do Mercado Pago vem com a cor azul (Mercado Pago Blue) por padrão no Brick. Embora passe credibilidade, foge um pouco do verde tático da marca. A API de customização do MP permite sobrescrever isso se desejado.
...
- _Observação:_ O loading do SDK do Mercado Pago demora cerca de 1s para aparecer o formulário na tela. Pode-se usar um Skeleton Loader para não deixar o espaço vazio enquanto a API do ML responde.
