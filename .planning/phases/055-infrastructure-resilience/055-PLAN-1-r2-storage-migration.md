...
class R2StorageService {
  ...
  async createPresignedUploadUrl({
    ...
    if (!this.s3) {
      ...
      return {
        uploadUrl: mockUrl,
        publicUrl: `http://localhost:3002/uploads/mock/${key}`,
      };
    }
...
1. A detecção de MOCK deve checar `storageType !== 'r2'` E a URL de mock:

```typescript
if (!uploadUrl || uploadUrl.includes("/api/mock/")) {
  /* fallback legado */
}
```

1. Ao chamar `upload/complete`, mandar `key` e `publicUrl` em vez de `fileId`:
...
1. O `xhr.onload` para R2 não retorna JSON com `id`. O R2 retorna `200 OK` sem body (para PUT simples) ou `204`. Ajustar:
