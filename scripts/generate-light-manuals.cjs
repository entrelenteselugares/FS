...
const SCREENSHOTS_DIR = path.join(MANUAIS_DIR, 'screenshots-light');
...
function safeName(label, pageName) {
  return `light_${label}_${pageName}`;
}
...
async function run() {
  ...
  // Save global summary
  const summaryPath = path.join(MANUAIS_DIR, 'light_capture_summary.json');
  fs.writeFileSync(summaryPath, JSON.stringify(summaries, null, 2));
  console.log(`\n🎉 Captura modo claro concluída! Summary: ${summaryPath}`);
}
