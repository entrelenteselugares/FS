...
test.describe('🏁 Launch Certification Suite v3.2', () => {
  ...
  test('Certification: ADMINISTRADOR (Master Control)', async ({ page }) => {
    ...
    await expect(page.getByText(/Lista de Pedidos/i)).toBeVisible();
    ...
    await page.getByRole('button', { name: /Logística|Catálogo|Impressão/i }).first().click();
    await expect(page.getByText(/Catálogo de Impressão/i)).toBeVisible();
  ...
  test('Certification: PROFISSIONAL (Lifecycle & Reference)', async ({ page }) => {
    ...
    await expect(page.getByText(/Configuração do Evento|Dados do Evento/i).first()).toBeVisible();
